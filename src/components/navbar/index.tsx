        ...
        <section className="w-screen flex justify-center">
            <div className="w-full max-w-screen-xl flex flex-col justify-center flex-wrap items-center gap-2  p-4 w-1/1 md:flex-row md:justify-between">
                ...
                <ul className="flex gap-3 flex-wrap justify-center">
                    {items.map(item =>
                        <a href={`#${item.link}`}>
                            <li key={item.name} className="cursor-pointer duration-300 px-2 py-1 rounded-lg capitalize hover:shadow-inner hover:bg-lime-400 hover:shadow-lime-700 hover:text-white">
                                {item.name}
                            </li>
                        </a>
                    )}
