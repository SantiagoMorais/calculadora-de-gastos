            ...
            <div className="p-4 w-full max-w-screen-xl border-b border-t mb-4">
                ...
                <div className="w-full flex justify-center gap-2 flex-wrap">
                    {data.months.map(month =>
                        <button onClick={() => handleCurrentMonth(month)} className={`capitalize border flex-1 max-w-24 w-full px-2 rounded-md text-center duration-500  hover:shadow-inner hover:bg-lime-400 hover:shadow-lime-700 hover:text-white ${currentMonth === month && "bg-lime-400 shadow-lime-700 text-white shadow-inner"}`}>
                            {month}
                        </button>
                    )}
                </div>
